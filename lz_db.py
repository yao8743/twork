...
if not os.getenv('GITHUB_ACTIONS'):
    load_dotenv(dotenv_path='.game.env')
...
class MySQLPointManager:
    ...
    async def update_user_point(self, user_id: int, new_point: int):
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("UPDATE user SET point = %s WHERE user_id = %s", (new_point, user_id))
                await conn.commit()
    ...
    async def get_or_create_user(self, user_id: int) -> int:
        ...
        if point == 0:
            async with self.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    await cur.execute("INSERT IGNORE INTO user (user_id, point) VALUES (%s, 0)", (user_id,))
                    await conn.commit()
            return 100
        return point
